...
import pathlib
import ruamel.yaml
from rasa_sdk import utils  # type: ignore
from rasa_sdk.forms import FormValidationAction  # type: ignore
from rasa_sdk.events import (  # type: ignore
    SlotSet,
    EventType,
    LoopInterrupted,
...
custom_forms_config = (
    ruamel.yaml.safe_load(
        open(  # pylint: disable=consider-using-with
            f"{here}/custom_forms_config.yml",
            "r",
            encoding="utf-8",
        )
    )
...
class CustomFormValidationAction(FormValidationAction, metaclass=abc.ABCMeta):
    ...
    (-) In domain.yml define these slots with appropriate mappings:
    ...
    (-) In domain.yml, for each form, define the 'utter_{form}_AA_CONTINUE_FORM'
        response, using /affirm & /deny buttons.
    ...
    async def validate(
        ...
        events.extend(await super().get_validation_events(dispatcher, tracker, domain))
    ...
    async def validate_AA_CONTINUE_FORM(  # pylint: disable=invalid-name, no-self-use
        self,
        value: Text,
        ...
        return await self.continue_yes_or_no(CF_SLOT, value)

    async def continue_yes_or_no(  # pylint: disable=no-self-use
        self, slot_name: str, value: Text
    ) -> Dict[Text, Any]:
        """Validates to continue or not"""
        if value == "yes":
    ...
    async def validate_zz_confirm_form(  # pylint: disable=no-self-use
        self,
        value: Text,
        _dispatcher: CollectingDispatcher,
        _tracker: Tracker,
